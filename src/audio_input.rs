...
pub fn start_audio_stream(
    ...
    thread::spawn(move || {
        let device = match choose_input_device_by_index(default_microphone_index) {
            Some(d) => d,
            None => {
                eprintln!(
...
// chooses an input device by index
fn choose_input_device_by_index(index: usize) -> Option<Device> {
